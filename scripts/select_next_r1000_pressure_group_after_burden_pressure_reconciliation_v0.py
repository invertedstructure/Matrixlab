...
UNIT_ID = "SELECT_NEXT_R1000_PRESSURE_GROUP_AFTER_BURDEN_PRESSURE_RECONCILIATION_V0"
TARGET_UNIT_ID = "r1000_pressure_group_selection.after_burden_pressure_reconciliation.v0"
...
SELECTED_BURDEN_PRESSURE_GROUP_ID = "b9a7575a"

OUT_DIR = ROOT / "data" / "r1000_pressure_group_selection_after_burden_pressure_reconciliation_v0"
RECEIPT_DIR = ROOT / "data" / "r1000_pressure_group_selection_after_burden_pressure_reconciliation_v0_receipts"

SELECTED_GROUP_PATH = OUT_DIR / "r1000_selected_pressure_group_after_burden_pressure_reconciliation.json"
SELECTION_DECISION_PATH = OUT_DIR / "r1000_pressure_group_selection_after_burden_decision.json"
SELECTION_AUTHORITY_PACKET_PATH = OUT_DIR / "r1000_selected_pressure_group_after_burden_authority_packet.json"
SELECTION_TRANSITION_TRACE_PATH = OUT_DIR / "r1000_pressure_group_selection_after_burden_transition_trace.json"
SELECTION_REPORT_PATH = OUT_DIR / "r1000_pressure_group_selection_after_burden_report.json"
...
BURDEN_QUEUE_DECISION_PATH = ROOT / "data" / "r1000_pressure_queue_reconciliation_after_selected_burden_pressure_group_inspection_v0" / "r1000_selected_burden_pressure_queue_reconciliation_decision.json"
...
HUMAN_DECISION = {
    "decision": "SELECT_NEXT_R1000_PRESSURE_GROUP_AFTER_BURDEN_PRESSURE_RECONCILIATION",
    "scope": "consume burden-pressure queue reconciliation and select the emitted next group candidate only; do not inspect rows, run R1000, repair, or open hidden follow-up work",
    "source_burden_queue_reconciliation_receipt_id": SOURCE_BURDEN_QUEUE_RECONCILIATION_RECEIPT_ID,
    "authorized": [
        "consume selected burden queue reconciliation receipt",
        "consume next selectable group candidate",
        "select candidate as current pressure group",
...
def load_sources() -> Dict[str, Any]:
    return {
        ...
        "burden_queue_decision": read_json(BURDEN_QUEUE_DECISION_PATH),
...
def validate_sources(sources: Dict[str, Any]) -> List[str]:
    ...
    receipt = sources["burden_queue_receipt"]
    reconciliation = sources["burden_queue_reconciliation"]
    next_candidate = sources["burden_next_candidate"]
    decision = sources["burden_queue_decision"]

    if receipt.get("receipt_id") != SOURCE_BURDEN_QUEUE_RECONCILIATION_RECEIPT_ID:
    ...
    if reconciliation.get("queue_reconciled") is not True:
        failures.append("burden_queue_reconciliation_artifact_not_reconciled")
    if reconciliation.get("next_group_opened") is not False:
        failures.append("burden_queue_reconciliation_artifact_opened_group")
    ...
    if next_candidate.get("next_group_opened") is not False:
        failures.append("next_candidate_already_opened")
    if not isinstance(next_candidate.get("candidate_summary"), dict):
        failures.append("next_candidate_summary_missing")

    if decision.get("next_group_opened") is not False:
        failures.append("burden_queue_decision_already_opened_next_group")
...
def candidate_summary(next_candidate: Dict[str, Any]) -> Dict[str, Any]:
    summary = next_candidate.get("candidate_summary") or {}
    return {
...
def build_selected_group(next_candidate: Dict[str, Any]) -> Dict[str, Any]:
    summary = candidate_summary(next_candidate)
    selected_pressure_group_id = sha8({
        "source_burden_queue_reconciliation_receipt_id": SOURCE_BURDEN_QUEUE_RECONCILIATION_RECEIPT_ID,
        "candidate_summary": summary,
        "selection": "selected_after_burden_pressure_reconciliation",
    })

    return {
        "schema_version": "r1000_selected_pressure_group_after_burden_pressure_reconciliation_v0",
        "selected_pressure_group_id": selected_pressure_group_id,
        "source_burden_queue_reconciliation_receipt_id": SOURCE_BURDEN_QUEUE_RECONCILIATION_RECEIPT_ID,
        "source_next_candidate_ref": rel(BURDEN_NEXT_CANDIDATE_PATH),
        ...
        "selection_scope": "current pressure group selection only after selected burden reconciliation",
...
def build_selection_decision(selected: Dict[str, Any]) -> Dict[str, Any]:
    return {
        "schema_version": "r1000_pressure_group_selection_after_burden_decision_v0",
        "selection_decision_id": sha8({
            "selected_pressure_group_id": selected["selected_pressure_group_id"],
            "source_burden_queue_reconciliation_receipt_id": SOURCE_BURDEN_QUEUE_RECONCILIATION_RECEIPT_ID,
        }),
        "decision_status": "SELECT_NEXT_GROUP_AFTER_BURDEN_PRESSURE_RECONCILIATION",
        "selected_pressure_group_id": selected["selected_pressure_group_id"],
        ...
        "recommended_next_handling": "INSPECT_SELECTED_R1000_PRESSURE_GROUP_AFTER_BURDEN_PRESSURE_RECONCILIATION_V0",
        "terminal": {
            "type": "STOP",
            "stop_code": "STOP_R1000_PRESSURE_GROUP_SELECTED_AFTER_BURDEN_RECONCILIATION_INSPECTION_REQUIRED",
...
def build_authority_packet(selected: Dict[str, Any], decision: Dict[str, Any]) -> Dict[str, Any]:
    return {
        "schema_version": "r1000_selected_pressure_group_after_burden_authority_packet_v0",
        "selected_pressure_group_id": selected["selected_pressure_group_id"],
        "selected_group": selected["selected_group"],
        ...
        "allowed_next_unit_scope": [
            ...
            "materialize selected group inspection surface if authorized",
...
def build_transition_trace(selected: Dict[str, Any], decision: Dict[str, Any]) -> Dict[str, Any]:
    return {
        "schema_version": "r1000_pressure_group_selection_after_burden_transition_trace_v0",
        "selected_pressure_group_id": selected["selected_pressure_group_id"],
        "trace": [
            {
                "step": "consume_burden_reconciled_queue",
                "question": "burden queue reconciled and next candidate emitted",
                "answer": True,
...
def build_report(selected: Dict[str, Any], decision: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        "schema_version": "r1000_pressure_group_selection_after_burden_report_v0",
        "unit_id": UNIT_ID,
        "target_unit_id": TARGET_UNIT_ID,
        "source_burden_queue_reconciliation_receipt_id": SOURCE_BURDEN_QUEUE_RECONCILIATION_RECEIPT_ID,
        "selected_pressure_group_id": selected["selected_pressure_group_id"],
        "selected_parent_pressure_class": group["parent_pressure_class"],
        "selected_pressure_subtype": group["pressure_subtype"],
        "selected_halt_reason": group["halt_reason"],
        "selected_row_count": group["row_count"],
        "burden_queue_reconciliation_consumed_count": 1,
...
def validate_outputs(
    ...
    if not isinstance(selected.get("selected_group"), dict):
        failures.append("selected_group_summary_missing")
...
def validate_receipt(receipt: Dict[str, Any]) -> List[str]:
    ...
    if terminal.get("stop_code") != "STOP_R1000_PRESSURE_GROUP_SELECTED_AFTER_BURDEN_RECONCILIATION_INSPECTION_REQUIRED":
...
def main() -> int:
    ...
    acceptance_gate_results = {
        "SELECT_AFTER_BURDEN_0_BURDEN_QUEUE_CONSUMED": sources["burden_queue_receipt"]["receipt_id"] == SOURCE_BURDEN_QUEUE_RECONCILIATION_RECEIPT_ID and sources["burden_queue_receipt"]["gate"] == "PASS",
        "SELECT_AFTER_BURDEN_1_NEXT_CANDIDATE_CONSUMED": next_candidate.get("selection_status") == "CANDIDATE_ONLY_NOT_OPENED",
        "SELECT_AFTER_BURDEN_2_PRESSURE_GROUP_SELECTED": report["pressure_group_selected_count"] == 1,
        "SELECT_AFTER_BURDEN_3_SELECTED_GROUP_NOT_INSPECTED": report["selected_group_inspected_count"] == 0,
        "SELECT_AFTER_BURDEN_4_NO_R1000_RUN": report["r1000_run_executed_count"] == 0,
        "SELECT_AFTER_BURDEN_5_NO_REPAIR_OR_APPLICATION": report["repair_executed_count"] == 0 and report["proposal_applied_count"] == 0,
        "SELECT_AFTER_BURDEN_6_NO_FIELD_VALUE_OR_TAXONOMY_ACTION": report["target_field_filled_count"] == 0 and report["descriptor_value_assignment_count"] == 0 and report["field_value_invention_count"] == 0 and report["taxonomy_delta_proposal_emitted_count"] == 0,
        "SELECT_AFTER_BURDEN_7_NO_SOURCE_OR_RECEIPT_MUTATION": source_mutation_detected is False and report["source_mutation_count"] == 0 and report["existing_receipt_mutation_count"] == 0,
        "SELECT_AFTER_BURDEN_8_NO_OTHER_GROUP_OPENED": report["other_group_opened_count"] == 0,
        "SELECT_AFTER_BURDEN_9_NO_HIDDEN_NEXT_COMMAND": report["hidden_next_command_count"] == 0 and decision["terminal"]["next_command_goal"] is None,
    }
    ...
    guards = {
        "burden_queue_reconciliation_consumed": True,
    ...
    receipt_seed = {
        "unit_id": UNIT_ID,
        "source_burden_queue_reconciliation": SOURCE_BURDEN_QUEUE_RECONCILIATION_RECEIPT_ID,
        "selected_pressure_group_id": selected["selected_pressure_group_id"],
        "terminal": terminal,
    }
    ...
    receipt = {
        "schema_version": "r1000_pressure_group_selection_after_burden_pressure_reconciliation_receipt_v0",
        "receipt_type": "R1000_PRESSURE_GROUP_SELECTION_AFTER_BURDEN_PRESSURE_RECONCILIATION_RECEIPT",
        ...
        "pressure_group_selection_after_burden_summary": {
            "selection_result": "SELECTED_NOT_INSPECTED",
            "selected_pressure_group_id": selected["selected_pressure_group_id"],
            "selected_group": selected["selected_group"],
            "selected_group_inspected": False,
        ...
        "pressure_group_selection_after_burden_guards": guards,
        "terminal": terminal,
    ...
    print(f"pressure_group_selection_after_burden_receipt_id={receipt_id}")
    print(f"pressure_group_selection_after_burden_receipt_path=data/r1000_pressure_group_selection_after_burden_pressure_reconciliation_v0_receipts/{receipt_id}.json")
    print(f"selected_pressure_group_after_burden_path=data/r1000_pressure_group_selection_after_burden_pressure_reconciliation_v0/r1000_selected_pressure_group_after_burden_pressure_reconciliation.json")
