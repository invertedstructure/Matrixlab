...
TARGET_UNIT_ID = "runtime.practical_radius_expansion.synthetic.handoff_and_feedback_D_E.v0"
NEXT_UNIT_ID = "ROLLUP_C7_SYNTHETIC_RUNTIME_RADIUS_TESTS_V0"

LAYER = "RUNTIME / C7 / SYNTHETIC_TEST / HANDOFF_AND_FEEDBACK_D_E"
MODE = "RUN_SYNTHETIC_FIXTURES / HANDOFF_AND_FEEDBACK_ONLY / NO_LIVE_RUNTIME"
BUILD_MODE = "C7_SYNTHETIC_HANDOFF_AND_FEEDBACK_D_E_RUN_ONLY"

SOURCE_C7_NEG_HALTS_RECEIPT_ID = "be2430b4"
SOURCE_C7_RUNNER_RECEIPT_ID = "0b9012bf"

NEG_HALTS_RECEIPT_PATH = ROOT / "data/c7_synthetic_radius_negative_halts_b_c_v0_receipts/be2430b4.json"
HANDOFF_FEEDBACK_TARGET_PATH = ROOT / "data/c7_synthetic_radius_negative_halts_b_c_v0/c7_handoff_and_feedback_d_e_run_target_v0.json"
...
FIXTURE_D_PATH = ROOT / "data/c7_synthetic_runtime_radius_fixtures_v0/fixtures/sequence_D_cell1_accepted_build_handoff.json"
FIXTURE_E_PATH = ROOT / "data/c7_synthetic_runtime_radius_fixtures_v0/fixtures/sequence_E_missing_capability_feedback.json"
...
OUT_DIR = ROOT / "data/c7_synthetic_radius_handoff_and_feedback_d_e_v0"
RECEIPT_DIR = ROOT / "data/c7_synthetic_radius_handoff_and_feedback_d_e_v0_receipts"

BASIS_PATH = OUT_DIR / "c7_handoff_and_feedback_d_e_run_basis_v0.json"
RUN_RESULTS_PATH = OUT_DIR / "c7_handoff_and_feedback_d_e_run_results_v0.json"
RUN_ARTIFACT_LINKS_PATH = OUT_DIR / "c7_handoff_and_feedback_d_e_run_artifact_links_v0.json"
ASSERTION_REPORT_PATH = OUT_DIR / "c7_handoff_and_feedback_d_e_assertion_report_v0.json"
ROLLUP_TARGET_PATH = OUT_DIR / "c7_full_synthetic_rollup_target_v0.json"
ROLLUP_PATH = OUT_DIR / "c7_handoff_and_feedback_d_e_run_rollup_v0.json"
PROFILE_PATH = OUT_DIR / "c7_handoff_and_feedback_d_e_run_profile_v0.json"
TRACE_PATH = OUT_DIR / "c7_handoff_and_feedback_d_e_run_transition_trace.json"

EXPECTED_NEG_HALTS_STATUS = "TYPED_C7_SYNTHETIC_NEGATIVE_HALTS_B_C_PASS_HANDOFF_FEEDBACK_NEXT"
...
FIXTURES = [
    {
        ...
        "expected_stop_code": "STOP_DONE",
        "expected_steps_attempted": 4,
        "expected_steps_advanced": 3,
        "expected_steps_halted": 1,
        "expected_unit_feedback_records": 0,
    },
    {
        "fixture_id": "sequence_E_missing_capability_feedback",
        "expected_outcome": "RADIUS_BLOCKED_BY_MISSING_CAPABILITY",
        "expected_stop_code": "STOP_MISSING_CAPABILITY",
...
def main() -> int:
    ...
    required = [
        NEG_HALTS_RECEIPT_PATH,
        HANDOFF_FEEDBACK_TARGET_PATH,
    ...
    neg_receipt = read_json(NEG_HALTS_RECEIPT_PATH)
    neg_summary = neg_receipt.get("machine_readable_c7_negative_halts_b_c_summary", {})
    handoff_target = read_json(HANDOFF_FEEDBACK_TARGET_PATH)
    ...
    if neg_receipt.get("receipt_id") != SOURCE_C7_NEG_HALTS_RECEIPT_ID:
        failures.append(f"negative_halts_receipt_id_wrong:{neg_receipt.get('receipt_id')}")
    if neg_receipt.get("gate") != "PASS":
        failures.append(f"negative_halts_gate_wrong:{neg_receipt.get('gate')}")
    if neg_summary.get("status") != EXPECTED_NEG_HALTS_STATUS:
        failures.append(f"negative_halts_status_wrong:{neg_summary.get('status')}")
    if neg_summary.get("next_unit_id") != UNIT_ID:
        failures.append(f"negative_halts_next_unit_wrong:{neg_summary.get('next_unit_id')}")
    if neg_receipt.get("terminal", {}).get("type") != "ADVANCE":
        failures.append("negative_halts_terminal_not_advance")
    if neg_receipt.get("terminal", {}).get("next_unit_id") != UNIT_ID:
        failures.append("negative_halts_terminal_next_wrong")
    if neg_summary.get("ready_for_handoff_and_feedback_d_e") is not True:
        failures.append("negative_halts_not_ready_for_d_e")

    if handoff_target.get("target_status") != "HANDOFF_AND_FEEDBACK_D_E_NEXT":
        failures.append(f"handoff_target_status_wrong:{handoff_target.get('target_status')}")
    if handoff_target.get("next_unit_id") != UNIT_ID:
        failures.append(f"handoff_target_next_wrong:{handoff_target.get('next_unit_id')}")
    if handoff_target.get("synthetic_test_only") is not True:
        failures.append("handoff_target_not_synthetic_only")
    if handoff_target.get("does_not_authorize_live_runtime") is not True:
        failures.append("handoff_target_live_runtime_boundary_missing")
    ...
    for fx in FIXTURES:
        if fx["fixture_id"] not in handoff_target.get("fixture_ids", []):
            failures.append(f"fixture_missing_from_handoff_target:{fx['fixture_id']}")
        if handoff_target.get("expected_outcomes", {}).get(fx["fixture_id"]) != fx["expected_outcome"]:
            failures.append(f"handoff_target_expected_wrong:{fx['fixture_id']}")
    ...
    status = "TYPED_C7_SYNTHETIC_HANDOFF_FEEDBACK_D_E_PASS_FULL_ROLLUP_NEXT" if gate == "PASS" else "TYPED_C7_SYNTHETIC_HANDOFF_FEEDBACK_D_E_GATE_FAIL"

    source_hash_manifest = {rel(p): file_sha256(p) for p in required}

    basis = {
        "schema_version": "c7_handoff_and_feedback_d_e_run_basis_v0",
        "unit_id": UNIT_ID,
        "basis_status": "BASIS_ACCEPTED" if gate == "PASS" else "BASIS_REPAIR_REQUIRED",
        "source_c7_negative_halts_receipt_id": SOURCE_C7_NEG_HALTS_RECEIPT_ID,
        "source_c7_runner_receipt_id": SOURCE_C7_RUNNER_RECEIPT_ID,
        "source_files": source_hash_manifest,
        "basis_claim": "Negative halts B/C passed and named handoff/feedback D/E as the next typed unit.",
        "does_not_authorize": [
            "live runtime run",
            "runtime adoption",
            "runtime patching",
    ...
    run_results_doc = {
        "schema_version": "c7_handoff_and_feedback_d_e_run_results_v0",
        "run_status": status,
    ...
    artifact_links = {
        "schema_version": "c7_handoff_and_feedback_d_e_run_artifact_links_v0",
        "artifact_status": "EMITTED" if gate == "PASS" else "PARTIAL",
        "negative_halts_receipt": rel(NEG_HALTS_RECEIPT_PATH),
    ...
    assertion_report = {
        "schema_version": "c7_handoff_and_feedback_d_e_assertion_report_v0",
        "assertion_status": "PASS" if gate == "PASS" else "FAIL",
        "failures": failures,
        "assertions": {
            "sequence_D_outcome_typed_boundaries": any(r["fixture_id"] == "sequence_D_cell1_accepted_build_handoff" and r["observed_outcome"] == "RADIUS_EXPANDED_WITH_TYPED_BOUNDARIES" for r in compact_results),
            "sequence_E_outcome_missing_capability": any(r["fixture_id"] == "sequence_E_missing_capability_feedback" and r["observed_outcome"] == "RADIUS_BLOCKED_BY_MISSING_CAPABILITY" for r in compact_results),
            ...
            "missing_capability_feedback_useful": any("USEFUL" in r.get("diagnostic_quality_values", []) for r in compact_results if r["fixture_id"] == "sequence_E_missing_capability_feedback"),
            "synthetic_only": True,
    ...
    rollup_target = {
        "schema_version": "c7_full_synthetic_rollup_target_v0",
        "target_status": "FULL_SYNTHETIC_ROLLUP_NEXT" if gate == "PASS" else "NOT_READY",
    ...
    rollup = {
        "schema_version": "c7_handoff_and_feedback_d_e_run_rollup_v0",
        "unit_id": UNIT_ID,
        "gate": gate,
        "status": status,
        "failures": failures,
        ...
        "ready_for_full_c7_synthetic_rollup": gate == "PASS",
        "next_unit_id": NEXT_UNIT_ID if gate == "PASS" else None,
    ...
    profile = {
        "schema_version": "c7_handoff_and_feedback_d_e_run_profile_v0",
        "profile_status": status,
        "profile": "C7_SYNTHETIC_HANDOFF_AND_FEEDBACK_D_E",
        "what_changed": "Synthetic Cell 1 accepted handoff and missing-capability feedback fixtures were run and classified.",
        "what_did_not_change": [
        ...
        "recommended_next": NEXT_UNIT_ID if gate == "PASS" else "REPAIR_C7_SYNTHETIC_HANDOFF_AND_FEEDBACK_D_E_RUN_V0",
        "next_command_goal": None,
    }

    trace = {
        "schema_version": "c7_handoff_and_feedback_d_e_run_transition_trace_v0",
        "unit_id": UNIT_ID,
        "transitions": [
            {
                "from": "C7_NEGATIVE_HALTS_B_C_PASS",
                "edge": "consume handoff/feedback target",
                "to": "C7_HANDOFF_FEEDBACK_D_E_BASIS_ACCEPTED" if gate == "PASS" else "C7_HANDOFF_FEEDBACK_D_E_BASIS_FAIL",
            },
            {
                "from": "C7_HANDOFF_FEEDBACK_D_E_BASIS_ACCEPTED" if gate == "PASS" else "C7_HANDOFF_FEEDBACK_D_E_BASIS_FAIL",
                "edge": "run handoff and missing-capability synthetic fixtures",
                "to": "C7_HANDOFF_FEEDBACK_D_E_PASS" if gate == "PASS" else "STOP_GATE_FAIL",
            },
        ],
        "terminal": {
            "type": "ADVANCE" if gate == "PASS" else "STOP",
            "next_unit_id": NEXT_UNIT_ID if gate == "PASS" else None,
            "stop_code": None if gate == "PASS" else "STOP_C7_HANDOFF_FEEDBACK_D_E_RUN_GATE_FAIL",
        },
    }
    ...
    for path, obj in [
        ...
        (ROLLUP_TARGET_PATH, rollup_target),
        (ROLLUP_PATH, rollup),
    ...
    reason_codes = [
        "C7_SYNTHETIC_HANDOFF_AND_FEEDBACK_D_E_RUN",
        "NEGATIVE_HALTS_RECEIPT_CONSUMED",
        "RUNNER_RECEIPT_CONSUMED",
        "SEQUENCE_D_HANDOFF_MATCHED_TYPED_BOUNDARIES",
        "SEQUENCE_E_MISSING_CAPABILITY_MATCHED",
        "UNIT_FEEDBACK_RECORD_USEFUL",
        "BAD_COUNTERS_ZERO",
        "NO_UNTYPED_HALT",
        "FULL_SYNTHETIC_ROLLUP_IS_NEXT_TYPED_UNIT",
        "NO_LIVE_RUNTIME_RUN",
        "NO_RUNTIME_ADOPTION",
        "NO_RUNTIME_PATCH",
    ...
    receipt_body = {
        "schema_version": "c7_handoff_and_feedback_d_e_receipt_v0",
        "receipt_type": "TYPED_C7_SYNTHETIC_HANDOFF_AND_FEEDBACK_D_E_RECEIPT",
        "created_at": now_iso(),
        "unit_id": UNIT_ID,
        ...
        "acceptance_gate_results": {
            "C7_HANDOFF_FEEDBACK_0_NEGATIVE_HALTS_RECEIPT_CONSUMED": gate == "PASS",
            "C7_HANDOFF_FEEDBACK_1_RUNNER_RECEIPT_CONSUMED": gate == "PASS",
            "C7_HANDOFF_FEEDBACK_2_SEQUENCE_D_HANDOFF_MATCHED": gate == "PASS",
            "C7_HANDOFF_FEEDBACK_3_SEQUENCE_E_MISSING_CAPABILITY_MATCHED": gate == "PASS",
            "C7_HANDOFF_FEEDBACK_4_FEEDBACK_RECORD_USEFUL": gate == "PASS",
            "C7_HANDOFF_FEEDBACK_5_BAD_COUNTERS_ZERO": gate == "PASS",
            "C7_HANDOFF_FEEDBACK_6_NO_UNTYPED_HALT": gate == "PASS",
            "C7_HANDOFF_FEEDBACK_7_FULL_ROLLUP_NEXT": gate == "PASS",
            "C7_HANDOFF_FEEDBACK_8_NO_LIVE_RUNTIME_RUN": gate == "PASS",
            "C7_HANDOFF_FEEDBACK_9_NO_HIDDEN_NEXT_COMMAND": gate == "PASS",
        },
        "machine_readable_c7_handoff_and_feedback_d_e_summary": {
            "status": status,
            "c7_handoff_and_feedback_d_e_run_done": gate == "PASS",
        ...
        "output_artifacts": {
            ...
            "rollup_target": rel(ROLLUP_TARGET_PATH),
    ...
    print(f"c7_handoff_feedback_d_e_receipt_id={receipt_id}")
    print(f"c7_handoff_feedback_d_e_receipt_path={rel(receipt_path)}")
    print(f"c7_handoff_feedback_d_e_next_unit={NEXT_UNIT_ID if gate == 'PASS' else 'NONE'}")
