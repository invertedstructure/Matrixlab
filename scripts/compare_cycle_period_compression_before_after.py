...
RECEIPT_DIR = ROOT / "data" / "cycle_period_compression_before_after_comparison_receipts"
...
def verify_cycle_period_observation(after: dict[str, Any]) -> list[str]:
    failures: list[str] = []
    obs = after.get("cycle_period_compression") or {}

    if obs.get("status") != "APPLIED_POST_EXECUTION_METADATA_ONLY":
        failures.append(f"cycle_period_status_wrong:{obs.get('status')}")
    if obs.get("certificate_scope") != "post_execution_observation_metadata_only":
        failures.append(f"cycle_period_scope_wrong:{obs.get('certificate_scope')}")
    if obs.get("certificate_count") != 5:
        failures.append(f"cycle_period_certificate_count_wrong:{obs.get('certificate_count')}")
    if obs.get("expected_cycle_slot_ids") != EXPECTED_CYCLE_SLOTS:
        failures.append(f"cycle_period_expected_slots_wrong:{obs.get('expected_cycle_slot_ids')}")
    if obs.get("observed_cycle_slot_ids") != EXPECTED_CYCLE_SLOTS:
        failures.append(f"cycle_period_observed_slots_wrong:{obs.get('observed_cycle_slot_ids')}")
    ...
    if obs.get("does_not_change_cycles_per_case") is not True:
        failures.append("cycle_period_cycles_per_case_changed")
    if obs.get("does_not_change_halt_reason") is not True:
        failures.append("cycle_period_halt_reason_changed")
    if obs.get("does_not_skip_cycle_execution") is not True:
        failures.append("cycle_period_skips_cycle_execution")
    if obs.get("does_not_early_halt_on_period_detection") is not True:
        failures.append("cycle_period_early_halts_on_period_detection")
    if obs.get("does_not_emit_synthetic_receipts") is not True:
        failures.append("cycle_period_emits_synthetic_receipts")
    if obs.get("does_not_reuse_prior_cycle_results_as_execution") is not True:
    ...
    for key in [
        ...
        if sem.get(key) is not False:
            failures.append(f"cycle_period_semantics_not_preserved:{key}:{sem.get(key)}")

    certificates = obs.get("certificates") or []
    if len(certificates) != 5:
        failures.append(f"cycle_period_certificate_rows_not_5:{len(certificates)}")
    else:
        cert_slots = [c.get("slot_id") for c in certificates]
        if cert_slots != EXPECTED_CYCLE_SLOTS:
            failures.append(f"cycle_period_certificate_slots_wrong:{cert_slots}")
        if any(c.get("metadata_only_no_execution_substitution") is not True for c in certificates):
            failures.append("cycle_period_certificate_missing_no_execution_substitution_marker")
...
def verify_sources(before: dict[str, Any], after: dict[str, Any], app: dict[str, Any]) -> list[str]:
    ...
    failures.extend(verify_cycle_period_observation(after))
...
def build_comparison(before_id: str, after_id: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if before_target["slots"] != EXPECTED_CYCLE_SLOTS:
        failures.append(f"before_target_slots_changed:{before_target['slots']}")
    if after_target["slots"] != EXPECTED_CYCLE_SLOTS:
    ...
    comparison = {
        "schema_version": "cycle_period_compression_before_after_comparison_v0",
        "comparison_kind": "CYCLE_PERIOD_COMPRESSION_BEFORE_AFTER_COMPARISON",
        ...
        "cycle_period_observation": after.get("cycle_period_compression") or {},
        "gate_rules": {
            ...
            "cycle_slot_identity_preserved": after_target["slots"] == EXPECTED_CYCLE_SLOTS,
            "target_receipts_preserved": after_target["receipts"] == before_target["receipts"],
            "target_cycle_elapsed_non_worse": after_elapsed <= before_elapsed,
            "cycle_period_metadata_present": (after.get("cycle_period_compression") or {}).get("status") == "APPLIED_POST_EXECUTION_METADATA_ONLY",
            "cycle_period_certificate_count_5": (after.get("cycle_period_compression") or {}).get("certificate_count") == 5,
            "cycle_period_metadata_only_no_execution_substitution": all(
                c.get("metadata_only_no_execution_substitution") is True
                for c in ((after.get("cycle_period_compression") or {}).get("certificates") or [])
            ),
        ...
        "terminal": {
            ...
            "next_command_goal": "DECIDE_CYCLE_PERIOD_COMPRESSION_FIX_OUTCOME_OR_NEXT_BURDEN_CLASS_V0" if not failures else None,
            "stop_code": None if not failures else "STOP_CYCLE_PERIOD_COMPRESSION_COMPARISON_FAILED",
        },
        "failures": failures,
        "warnings": warnings,
    ...
    receipt = {
        "schema_version": "cycle_period_compression_before_after_comparison_receipt_v0",
        "comparison_id": sig,
        "comparison_path": f"data/cycle_period_compression_before_after_comparisons/{sig}.json",
        "comparison_sig8": sig,
        "source_application_id": EXPECTED_APPLICATION_ID,
        "source_candidate_id": EXPECTED_SOURCE_CANDIDATE_ID,
        "source_policy_id": EXPECTED_SOURCE_POLICY_ID,
        ...
        "target_receipts_preserved": after_target["receipts"] == before_target["receipts"],
        "cycle_period_metadata_present": (after.get("cycle_period_compression") or {}).get("status") == "APPLIED_POST_EXECUTION_METADATA_ONLY",
        "cycle_period_certificate_count": (after.get("cycle_period_compression") or {}).get("certificate_count"),
...
def main() -> int:
    ...
    print(f"comparison_json_path=data/cycle_period_compression_before_after_comparisons/{comparison['comparison_id']}.json")
    print(f"comparison_receipt_path=data/cycle_period_compression_before_after_comparison_receipts/{comparison['comparison_id']}.json")
