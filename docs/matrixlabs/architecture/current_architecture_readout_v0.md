# Current MatrixLabs Architecture Readout v0

This is a careful readout of visible source-backed MatrixLabs architecture. It is not a redesign, not a schema promotion, and not reusable authorization. When the evidence only partially supports a concept, this readout marks it as partial or uncertain.

## Current source posture

The active repo is `/home/asd/projects/matrixlab` in the `MatrixLabRescue` WSL distro. The repo is strongly packet/receipt oriented: `data/` contains 1015 top-level packet/data directories and 466 top-level receipt directories; `/home/asd/matrixlab_receipts/` supplied 714 copied external receipt files. `scripts/` contains 455 unit scripts, but no project runtime/probe/build/rerun commands were run during this extraction.

Pre-existing git state included modified/staged files in `.gitignore`, a C8 post-patch surface decision acceptance chain, and one C8 acceptance script. This extraction did not alter those source artifacts.
...
## Load-bearing receipt information

Load-bearing receipt fields observed across chains include receipt ID, receipt type, schema version, unit ID, target unit ID, gate/status, terminal stop code, source receipt IDs, source artifact paths, boundary audit IDs, negative controls, counters, scope, allowed actions, forbidden actions, and next recommended review/decision surfaces.

## Recurring decision graph compression candidates
...
## Uncertain or partial concepts

- Phase7 was not found as an explicit visible source surface in this pass.
- `scribe` naming needs source confirmation as a formal component.
- Any concept present only in terminal/chat residue remains residue-backed, not yet source-backed, until copied into `raw/` and mapped to repo evidence.
